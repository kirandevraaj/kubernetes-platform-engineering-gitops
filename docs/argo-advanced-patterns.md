...
### Live observations (VMware Argo CD v3.5.3 / ckad-lab)

| Experiment | Observed |
|---|---|
| AppProject wrong namespace (kube-system) | InvalidSpecError: destination does not match allowed destinations in platform-advanced-lab |
| Combined sync (phases+waves+hooks) | Order: Namespace → PreSync Job → ConfigMap → Service → Deployment → PostSync Job; Application Synced/Healthy; hook Jobs cleaned (HookSucceeded) |
| SyncFail demo | PreSync Job failed (ackoff limit); SyncFail Job succeeded; operation phase Failed; main lab App remained Synced/Healthy after demo App deleted |
| Health-gated waves | Sync applied wave-0 dependency before wave-1 dependent; selfHeal restored Git image after live drift; Deployment readiness from Git determines Healthy |
| Sync Window deny | Operation message: Sync operation blocked by sync window; windows cleared afterward (syncWindows: []) |
| Manual delete generated App | rgo-advanced-stage regenerated by ApplicationSet owner (platform-advanced-set) |
| Remove generator item stage | Generated Application removed; restoring list elements recreated rgo-advanced-stage |
| Cluster generator | Label-selected Secret platform-lab.io/advanced-lab=true produced cluster-advanced-in-cluster-advanced-lab (in-cluster only; AWS not registered on VMware Argo) |
| Git generator | Temporarily generated git-advanced-{dev,test,stage}; ApplicationSet then deleted to avoid dual ownership |
| Idempotent re-sync | PreSync + PostSync Jobs ran again successfully; Jobs absent afterward (delete policy) |
| App-of-Apps | Parent platform-app-of-apps Synced; children pp-child-* Synced/Healthy |
| App-of-ApplicationSets | Parent Synced; platform-nested-list-set generated 
ested-advanced-* without recursion |
| Ownership | Generated Apps have ownerReferences → ApplicationSet; child Apps own workload resources |
