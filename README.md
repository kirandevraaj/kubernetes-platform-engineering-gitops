...
9. **Networking.** Completed for the local lab. `kubernetes/base` includes Ingress `platform-lab` (`platform-lab.local` → Service `:8000` via ingress-nginx) and a namespace-scoped NetworkPolicy allowing TCP/8000 only from the ingress-nginx controller. Reconciled by Argo CD. See [docs/architecture/networking.md](docs/architecture/networking.md).
